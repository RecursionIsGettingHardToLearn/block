...
interface Props {
  /** Título del panel. Por defecto, orientado al votante. */
  titulo?: string;
...
/**
 * Verificación de un voto por su ID de transacción o subiendo la imagen del QR
 * (se lee con jsQR y se extrae el txId). Consulta GET /fabric/verify/:txId, que
 * no requiere rol, así que sirve igual para el votante (verificar su propio
 * voto) y para el auditor (validar un voto que le presentan). Los textos se
 * ajustan con props para cada caso.
 */
export default function VoteVerifier({
  titulo = 'Verifica tu voto',
  descripcion = 'Pega tu ID de transacción o sube la imagen del QR que descargaste al votar, para confirmar que tu voto quedó registrado.',
}: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
      ...
      <p className="text-xs text-slate-400 mb-4">{descripcion}</p>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Pega el ID de transacción…"
